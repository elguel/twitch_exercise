...
def add_foreign_keys(engine):
    engine.execute(
        f'alter table chat_fact ADD CONSTRAINT fk_chat_commenters FOREIGN KEY (commenter_id) REFERENCES commenter_dimension (commenter_id)')
...
def twitch_etl():
    ...
    dtype_dic_chat = {'body': types.String(), 'channel_id': types.INTEGER(), 'commenter_id': types.INTEGER(), 'created_at': types.DateTime(), 'offset': types.DECIMAL,
                      'updated_at': types.DateTime(), 'video_id': types.INTEGER, 'emoticon_id': types.INTEGER, 'text': types.String(), 'unique_id': types.INTEGER, 'polarity': types.FLOAT}
    dtype_dic_commenter = {
